...
class Router {
    ...
    _register_recursive(method: String, path_elems: Array<String>, handler: Function, previousRoute: Route): Route | undefined {
        ...
        if (path_elems.length == 1) {
            ...
            let existingRoute = previousRoute.subRoutes.find(route => route.method === method && route.path === currentPathElem);
            if (existingRoute){
            ...
            // FIXME: only create a new route if there are no existing routes at this level with the same method and path
            let existingRoute = previousRoute.subRoutes.find(route => route.method === method && route.path === currentPathElem);
            let currentPathRoute = existingRoute ? existingRoute : new Route(method, currentPathElem!);
            let subroute = this._register_recursive(method, path_elems, handler, currentPathRoute);
            let route;
            if (subroute) {
    ...
    register(method: String, path: String, handler: Function) {
        ...
        if (path === '/' || path === '') {
            if (this._routes.find(route=>route.method === method && route.path === '/' && route.handler === handler)){
                throw Error(`Route has already been registered with ${method} ${path} ${handler}`)
            } else {
            ...
            let subroute = this._register_recursive(method, path_elems, handler, rootRouteForThisMethod)!;
            return rootRouteForThisMethod.addSubRoute(subroute);
        }
    }


    _recursive_route(request: Request, request_method: String, request_path_elems: Array<String>, traversedRoute: Route, parameterMap: any, ignoreRoutes: Array<Route>): Route | undefined {
        if (request_path_elems.length == 1) {
            ...
            return (subroute && !ignoreRoutes.includes(subroute)) ? subroute : undefined;
            ...
            if (subroute){
                let route = this._recursive_route(request, request_method, request_path_elems, subroute, parameterMap, ignoreRoutes);
                return route;
            } else {
                return undefined;
            }
    ...
    route(req: http.IncomingMessage) {
        ...
        if (request.url?.trim() === '/' || request.url?.trim() === '') {
            ...
            while (route != undefined && route.handler == undefined){
                ...
                console.log('trying');
                route = this._recursive_route(request, request.method!, request_path_elems, rootRoute!, {}, improper_matches);
